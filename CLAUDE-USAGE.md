...
```json
{"statusLine":{"type":"command","command":"C:/Users/gust123/Documents/ai-usage/aiusage.exe claude-statusline"}}
```

若已經有 statusLine，請整合既有命令，避免覆蓋原本功能。程式不會自動改動 Claude 設定。
...
使用 `golang:latest`；在專案目錄執行：

```powershell
docker run --rm --pull=always -v "${PWD}:/src" -w /src golang:latest sh -c "go test ./... && GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -trimpath -ldflags='-s -w' -o dist/aiusage.exe . && GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -trimpath -ldflags='-s -w -H windowsgui' -o dist/aiusage-panel.exe ."
```
