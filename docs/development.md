...
本文件是給維護者看的：建置與發行流程、面板與額度的計量規則、設定檔欄位、
疑難排解、資料邊界與架構。一般使用方式請看 [README](../README.md)。
...
- [Claude 用量來源與 statusLine 設定](providers/claude.md)
- [Cursor 登入來源、計量與重試規則](providers/cursor.md)
- [Antigravity 模型群額度與本機服務](providers/antigravity.md)
...
**每次發行後要手動更新 [README](../README.md) 的下載表格**：資產檔名內含版本號
（`aiusage-vX.Y.Z-<平台>.tar.gz`），GitHub 的 `releases/latest/download/` 轉址幫不上忙，
所以那六個連結與版本字樣必須跟著新 tag 一起改，否則使用者下載到的是舊版。
...
app 模式使用獨立的 `panel-browser` 設定資料夾。面板是一條垂直的儀表板：
各來源由上往下堆疊，一個來源一張卡，卡片寬度固定 200 px，所以視窗寬度固定、
高度依偵測到的來源數量決定。四個來源（含 Cursor）時是 236 × 595，
這個高度是實測出來的，全部收合的預設狀態不會出現垂直捲軸；展開詳細資訊時才會捲動。
Windows 面板會鎖定這個尺寸並設為最上層顯示。
面板右上角有重新掃描按鈕；最小化與關閉使用瀏覽器標題列的控制項。
擺放位置可以拖曳標題列，也可以直接在面板的空白處按住拖動 ——
按鈕、輸入框與展開的詳細資訊除外，那些地方仍然照常操作。
面板平常為 92% 不透明，滑鼠指標移到面板上時變成完全不透明，移開後自動淡回去；
卡片本身是半透明的玻璃質感，透明感主要由卡片負責，整個視窗不會連文字一起變淡。
視窗樣式由背景每秒重新套用，因此瀏覽器還原樣式後會自動修正。
...
每張卡只留儀表板該有的東西：來源圖示與名稱、重置倒數、已使用百分比、進度條與用量。
**在卡片上按一下左鍵**即可展開或收合詳細資訊：計量視窗、剩餘比例、其他限制、來源時間、
錯誤訊息及上限設定。展開中的卡片會亮起來，一眼看得出哪一張是開著的；
展開後點卡片內的輸入框與按鈕不會把它收起來。鍵盤可用 Tab 聚焦後按 Enter 或空白鍵。
...
- **多重限制**：Codex 通常顯示較短視窗，其他視窗放在詳細資訊；Claude 會列出來源提供的
  ...
  Cursor 顯示帳單週期，並在卡內直接列出 Cursor model、Other model 各自的百分比與進度條；
  方案總百分比與各池金額移至詳細資訊，缺少分項資料顯示 `—%`。
- **重置時間**：有官方重置時間時採用回報值。本機滾動視窗的「最舊紀錄滑出」
  只表示一筆紀錄即將離開統計範圍，不表示帳號額度全部重置。
- **重置倒數**：來源名稱旁的倒數每秒更新，只採用帳號回報的重置時間；
  來源沒有提供時顯示「重置未知」，不會拿本機滾動視窗的時間充數。
  倒數歸零後面板會立即重查一次該來源的帳號額度，取得新額度前顯示「等待更新」。
- **警戒與狀態**：低於警戒線使用來源色，達警戒線轉黃，達 100% 轉紅；
  各來源以自家圖示標示，沒有可用資料時轉為淡色。速率依最近 15 分鐘的本機事件估算。
...
詳細的 SOLID 對應、來源擴充契約、依賴圖及已知限制見 [架構說明](architecture.md)。
新增來源透過 `Collector` registry 與 `ProviderPolicy` 接入，service 不依賴具體來源。
設定採深拷貝，只有儲存成功後才更新執行中的設定。

接續開發可先讀 [專案記憶](project-memory.md)，快速找到需求對應的程式位置、
計量規則與驗證方式；AI 協作入口為 [AGENTS.md](../AGENTS.md)。
檢視發現及測試範圍見 [2026-09-14 檢視紀錄](review-2026-09-14.md) 與
[2026-09-15 檢視紀錄](review-2026-09-15.md)。
