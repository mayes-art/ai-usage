...
func installedProviders() map[string]bool {
	...
	commands := map[string][]string{
		providerClaude: {"claude.exe", "claude.cmd"}, providerCodex: {"codex.exe", "codex.cmd"},
		providerCursor: {"cursor.exe", "cursor.cmd", "cursor-agent.exe"}, providerAntigravity: {"agy.exe", "antigravity.exe", "antigravity.cmd"},
	}
	...
	local, home := os.Getenv("LOCALAPPDATA"), userHome()
	paths := map[string][]string{
		providerClaude:      {filepath.Join(home, ".local", "bin", "claude.exe"), filepath.Join(local, "AnthropicClaude", "claude.exe"), filepath.Join(local, "Programs", "Claude", "Claude.exe")},
		providerCodex:       {filepath.Join(os.Getenv("APPDATA"), "npm", "codex.cmd")},
		providerCursor:      {filepath.Join(local, "Programs", "cursor", "Cursor.exe"), filepath.Join(home, ".local", "bin", "cursor-agent.exe")},
		providerAntigravity: {filepath.Join(local, "agy", "bin", "agy.exe"), filepath.Join(local, "Programs", "Antigravity", "Antigravity.exe")},
	}
	...
	if err != nil || json.Unmarshal(data, &names) != nil {
		...
		for _, name := range names {
			...
			for id, word := range map[string]string{providerClaude: "claude", providerCodex: "codex", providerCursor: "cursor", providerAntigravity: "antigravity"} {
				if strings.Contains(name, word) {
					found[id] = true
				}
			}
			if name == "agy" {
				found[providerAntigravity] = true
			}
		}
	}
