...
func usageFromMap(m map[string]any) (model.Usage, bool) {
	var u model.Usage
	matched := 0
	for k, v := range m {
		f, ok := ToNumber(v)
...
func classifyHits(hits []usageHit) (usage model.Usage, cumulative bool) {
...
// ---------------------------------------------------------------------------
// 從一筆記錄產生 model.Event / model.Reported
// ---------------------------------------------------------------------------

// ParseRecord 把一筆 JSON 記錄轉成事件。fallbackTime 通常是檔案的 mtime。
func ParseRecord(provider string, raw []byte, fallbackTime time.Time, collectKeys bool) (*model.Event, *model.Reported, map[string]int) {
	var node any
...
func parseNode(provider string, node any, fallbackTime time.Time, collectKeys bool, rawHash string) (*model.Event, *model.Reported, map[string]int) {
	hits, ctx := ExtractRecord(node, collectKeys)

	var rep *model.Reported
	...
	if provider == model.ProviderCodex {
		// Codex reports cached input within input tokens and reasoning within
		// output tokens. Retain those details without billing them twice.
		for i := range hits {
			u := &hits[i].usage
			if u.Total == 0 {
...
func BuildReported(ctx *extractCtx, at time.Time) *model.Reported {
	r := &model.Reported{At: at, Fields: map[string]float64{}, Labels: map[string]string{}}
	...
	groups := map[string]*model.ReportedWindow{}
