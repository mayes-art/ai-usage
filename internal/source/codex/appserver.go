...
func readCodexRateLimits(timeout time.Duration) (*model.Reported, error) {
	...
	if err := enc.Encode(map[string]any{
		...
		"params": map[string]any{"clientInfo": map[string]string{
			"name": "aiusage", "title": "AI Usage", "version": buildinfo.Version,
		}},
	}); err != nil {
	...
	for {
		...
		_, extract := record.ExtractRecord(node, false)
		reported := record.BuildReported(extract, time.Now())
