...
func parse(t *testing.T, provider, raw string) *Event {
	t.Helper()
...
func TestClaudeShape(t *testing.T) {
	...
	if ev.Key != providerClaude+"|u|req_1" {
...
func TestCumulativeDeltaInStore(t *testing.T) {
	cfg := DefaultConfig()
	s := NewStore(cfg)
	mk := func(in, out int64, uid string) Event {
		return Event{
			Provider: providerCodex, At: time.Now(), Session: "sx", Cumulative: true,
			Usage: Usage{Input: in, Output: out}, Key: "codex|u|" + uid,
		}
	}
	apply := func(ev Event) int64 {
		if _, dup := s.seen[ev.Key]; dup {
			return -1
		}
		s.seen[ev.Key] = struct{}{}
		ck := ev.Provider + "|" + ev.Session
		prev := s.cum[ck]
		delta := deltaUsage(prev, ev.Usage)
		s.cum[ck] = maxUsage(prev, ev.Usage)
		return delta.Billable()
	}
	if got := apply(mk(1000, 200, "a")); got != 1200 {
		t.Errorf("第一筆增量 = %d, 期望 1200", got)
	}
	if got := apply(mk(1500, 260, "b")); got != 560 {
		t.Errorf("第二筆增量 = %d, 期望 560", got)
	}
	if got := apply(mk(1500, 260, "c")); got != 0 {
		t.Errorf("累計沒變時增量應為 0, got %d", got)
	}
}
...
func TestFallbackTime(t *testing.T) {
	ev := parse(t, providerClaude, `{"usage":{"input_tokens":10,"output_tokens":20}}`)
	if ev == nil {
		t.Fatal("沒有解析出事件")
	}
...
func TestIgnoresNonUsage(t *testing.T) {
	for _, raw := range []string{
		...
		if ev := parse(t, providerClaude, raw); ev != nil {
...
func TestReportedQuota(t *testing.T) {
	...
	_, rep, _ := ParseRecord(providerClaude, []byte(raw), fallback, false)
	if rep == nil {
		t.Fatal("沒有抓到額度資訊")
	}
	if !rep.usable() {
...
func TestDedupeKeyStable(t *testing.T) {
	...
	a := parse(t, providerClaude, raw)
	b := parse(t, providerClaude, raw)
	if a.Key != b.Key {
...
func TestHashFallbackKey(t *testing.T) {
	a := parse(t, providerGemini, `{"usageMetadata":{"promptTokenCount":1,"candidatesTokenCount":2}}`)
	b := parse(t, providerGemini, `{"usageMetadata":{"promptTokenCount":3,"candidatesTokenCount":4}}`)
	if a.Key == b.Key {
...
func TestWindowBounds(t *testing.T) {
	...
	rolling := ProviderConfig{WindowKind: "rolling", WindowSeconds: 5 * 3600}
	s, e, label := windowBounds(rolling, now)
	if now.Sub(s) != 5*time.Hour || !e.IsZero() {
		t.Errorf("滾動視窗錯誤: %v %v", s, e)
	}
	if label != "5 小時滾動視窗" {
		t.Errorf("label = %q", label)
	}

	day := ProviderConfig{WindowKind: "day"}
	s, e, _ = windowBounds(day, now)
	if s.Hour() != 0 || e.Day() != 12 {
		t.Errorf("日視窗錯誤: %v %v", s, e)
	}

	month := ProviderConfig{WindowKind: "month"}
	s, e, _ = windowBounds(month, now)
	if s.Day() != 1 || e.Month() != time.October {
		t.Errorf("月視窗錯誤: %v %v", s, e)
	}
}
...
func TestCodexDualRateLimitWindows(t *testing.T) {
	...
	for i := 0; i < 200; i++ {
		_, rep, _ := ParseRecord(providerCodex, []byte(raw), fallback, false)
...
func TestReportedResetBeatsRollingGuess(t *testing.T) {
	cfg := DefaultConfig()
	cfg.PreferReported = true
	p := cfg.provider(providerCodex)
	p.Roots = nil

	s := NewStore(cfg)
	now := time.Now()
	reset := now.Add(2 * time.Hour)
	s.reported[providerCodex] = &Reported{
		At:      now.Add(-5 * time.Minute),
		Windows: []ReportedWindow{{Label: "5 小時", PercentUsed: 93, WindowMinutes: 300, ResetAt: &reset}},
	}
	pv := 93.0
	s.reported[providerCodex].PercentUsed = &pv
	s.reported[providerCodex].ResetAt = &reset
	s.events[providerCodex] = []Event{{
		Provider: providerCodex, At: now.Add(-4 * time.Hour),
		Usage: Usage{Input: 100, Output: 50}, Key: "k1",
	}}
	s.stats[providerCodex] = &scanStats{Files: 1, RootsHit: []string{"x"}}

	ps := s.providerSnapshot(*p, now)
	if !ps.HasLimit || ps.Percent != 0.93 {
		t.Fatalf("百分比 = %v，期望 0.93", ps.Percent)
	}
	if ps.ResetLabel != "重置" {
		t.Errorf("ResetLabel = %q，期望「重置」（官方回報值）", ps.ResetLabel)
	}
	if ps.ResetAt == nil || *ps.ResetAt != reset.Unix() {
		t.Errorf("ResetAt = %v，期望官方回報的 %d", ps.ResetAt, reset.Unix())
	}
	if ps.WindowLabel != "5 小時窗" {
		t.Errorf("WindowLabel = %q，期望「5 小時窗」", ps.WindowLabel)
	}
}
